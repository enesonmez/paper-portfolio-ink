...
- [ ] React Router v7 projesinin (Edge Runtime ayarlı) başlatılması.
- [ ] Tailwind CSS ve shadcn/ui entegrasyonunun yapılması.
- [ ] Cloudflare `wrangler.toml` dosyasının oluşturulması ve D1 veritabanı binding ayarlarının yapılması.
...
- [ ] Drizzle ORM kurulumu ve D1 veritabanı adaptörünün bağlanması.
- [ ] Temel veritabanı şemalarının (`users`, `posts`, `projects`, `sessions`) `schema.ts` içinde oluşturulması.
- [ ] Local geliştirme için ilk D1 migration (göç) işleminin başarıyla çalıştırılması.
