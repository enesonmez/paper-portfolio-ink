...
import type * as UserFormServerModule from "../../app/lib/users/user-form.server";
...
vi.mock("../../app/lib/users/user-form.server", async () => {
  const actual = await vi.importActual<typeof UserFormServerModule>(
    "../../app/lib/users/user-form.server",
  );
...
describe("dashboard users server", () => {
  ...
  it("loads the users registry for admin sessions", async () => {
    const { loadDashboardUsersData } =
      await import("../../app/features/dashboard/users/server");
