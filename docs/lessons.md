...
- En okunabilir dashboard deseni, `handleDashboard*Action` orkestrasyonunu `server.ts` icinde birakip `handleCreate*Mutation`, `handleUpdate*Mutation`, `handleDelete*Mutation` gibi helper'lari `mutations.server.ts` dosyasina tasimak oldu; boylece route-facing kararlar tek dosyada kalirken CRUD ve side-effect detaylari feature-yerel ama izole okunabiliyor.
